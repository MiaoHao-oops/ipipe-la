...
{
	COMMENT(" PM offsets. ");
	OFFSET(SSS_SP,		loongarch_static_suspend_state, sp);
	BLANK();
}
#endif
...
void output_kvm_defines(void)
{
	...
	OFFSET(VCPU_VCSR, kvm_vcpu_arch, fpu.vcsr);
	BLANK();
	...
	OFFSET(KVM_ARCH_HCRMD, kvm_vcpu_arch, host_crmd);
	OFFSET(KVM_ARCH_HPRMD, kvm_vcpu_arch, host_prmd);
	OFFSET(KVM_ARCH_HESTAT, kvm_vcpu_arch, host_estat);
	OFFSET(KVM_ARCH_HBADV, kvm_vcpu_arch, host_badvaddr);
	OFFSET(KVM_ARCH_HBADI, kvm_vcpu_arch, host_badinstr);
