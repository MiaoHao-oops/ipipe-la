...
static inline int patch_tramp(struct module_ftrace_tramp *p, void *addr)
{
	...
	addu16id = la64_insn_gen_addu16id(t1, zero, ADDR_IMM(pos, ADDU16ID));
	lu32id = la64_insn_gen_lu32id(t1, ADDR_IMM(pos, LU32ID));
	lu52id = la64_insn_gen_lu52id(t1, t1, ADDR_IMM(pos, LU52ID));
	jirl = la64_insn_gen_jirl(zero, t1, 0, (pos & 0xffff));

	return la64_insn_patch_text(&p->addu16id, addu16id) ||
		  la64_insn_patch_text(&p->lu32id, lu32id) ||
		  la64_insn_patch_text(&p->lu52id, lu52id) ||
		  la64_insn_patch_text(&p->jirl, jirl);
