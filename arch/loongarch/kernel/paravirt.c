 ...
 *
 * Author: wangjianxing <wangjianxing@loongson.cn>
 */
...
phys_addr_t slow_virt_to_phys(const void *vaddr)
{
	...
	/* See arch/loongarch/la64/numa.c:setup_per_cpu_areas */
	if (nr_node_ids < 8)
...
int __init pv_ipi_init(void)
{
	...
	if (!(pv_feature_disabled & (1 << KVM_FEATURE_MULTI_IPI)) &&
		...
		mp_ops->send_ipi_mask = pv_send_ipi;
	}
	return 0;
}

static int __init set_pv_feature(char *str)
{
	if (!str)
		return -EINVAL;
	return kstrtouint(str, 16, &pv_feature_disabled);
}
early_param("pv_disable=", set_pv_feature);
