...
static void __init apply_r_loongarch_la_rel(u32 *loc_new, long offset)
{
	...
	dest = ori->reg2i12_format.simmediate & 0xfff;
	...
	ori->reg2i12_format.simmediate = dest & 0xfff;
...
static void (*reloc_handlers_rel[16]) (u32 *, long) __initdata = {
	[1]		= apply_r_loongarch_32_rel,
	[2]		= apply_r_loongarch_64_rel,
	[3]		= apply_r_loongarch_la_rel,
};
...
static int __init do_relocations(void *kbase_old, void *kbase_new, long offset)
{
	...
	for (r = _relocation_start; r < _relocation_end; r++) {
		...
		type = (*r >> 28) & 0xf;
		loc_orig = (void *)(kbase_old + ((*r & 0x0fffffff) << 2));
...
static inline void __init *determine_relocation_address(void)
{
	 ...
	 */
	return (void *)(CAC_BASE + 0x01000000);
}
