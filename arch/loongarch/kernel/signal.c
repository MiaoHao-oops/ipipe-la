...
static int copy_lbt_to_sigcontext(struct sigcontext __user *sc)
{
	...
	err |= __put_user(current->thread.scr0, &scrregs[0]);
	err |= __put_user(current->thread.scr1, &scrregs[1]);
	err |= __put_user(current->thread.scr2, &scrregs[2]);
	err |= __put_user(current->thread.scr3, &scrregs[3]);

	err |= __put_user(current->thread.eflag, eflags);
...
static int copy_lbt_from_sigcontext(struct sigcontext __user *sc)
{
	...
	err |= __get_user(current->thread.scr0, &scrregs[0]);
	err |= __get_user(current->thread.scr1, &scrregs[1]);
	err |= __get_user(current->thread.scr2, &scrregs[2]);
	err |= __get_user(current->thread.scr3, &scrregs[3]);

	err |= __get_user(current->thread.eflag, eflags);
