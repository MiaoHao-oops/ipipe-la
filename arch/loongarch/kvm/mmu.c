 ...
 * for more details.
 *
 * KVM/LOONGISA MMU handling in the KVM module.
 *
...
pgd_t *kvm_pgd_alloc(void)
{
	...
	ret = (pgd_t *)__get_free_pages(GFP_KERNEL, 0);
	if (ret)
		pgd_init(ret);
...
static int kvm_##name##_pte(pte_t *pte, unsigned long start,		\
				 unsigned long end)			\
{									\
	int ret = 0;							\
...
static int kvm_##name##_pmd(pmd_t *pmd, unsigned long start,		\
				 unsigned long end)			\
{									\
	int ret = 0;							\
	pte_t *pte;							\
	...
	for (i = i_min; i <= i_max; ++i, start = 0) {			\
		...
		if (i == i_max)						\
			cur_end = end;					\
									\
		ret |= kvm_##name##_pte(pte, start, cur_end);		\
	}								\
	return ret;							\
}									\
									\
static int kvm_##name##_pud(pud_t *pud, unsigned long start,		\
				 unsigned long end)			\
{									\
	int ret = 0;							\
	pmd_t *pmd;							\
	...
	for (i = i_min; i <= i_max; ++i, start = 0) {			\
		...
		if (i == i_max)						\
			cur_end = end;					\
									\
		ret |= kvm_##name##_pmd(pmd, start, cur_end);		\
	}								\
	return ret;							\
}									\
									\
static int kvm_##name##_pgd(pgd_t *pgd, unsigned long start,		\
				 unsigned long end)			\
{									\
	int ret = 0;							\
	pud_t *pud;							\
	...
	for (i = i_min; i <= i_max; ++i, start = 0) {			\
		...
		if (i == i_max)						\
			cur_end = end;					\
									\
		ret |= kvm_##name##_pud(pud, start, cur_end);		\
	}								\
	return ret;							\
}
...
BUILD_PTE_RANGE_OP(mkclean, pte_mkclean, kvm_pmd_mkclean)
		...
		enum kvm_mr_change change)
{
	...
	if (change == KVM_MR_FLAGS_ONLY &&
		...
		if (needs_flush)
			kvm->arch.kvm_ops->flush_shadow_memslot(kvm, new);
...
void kvm_arch_flush_shadow_all(struct kvm *kvm)
{
	...
	/* Let implementation do the rest */
	kvm->arch.kvm_ops->flush_shadow_all(kvm);
}
		...
		struct kvm_memory_slot *slot)
{
	...
	/* Let implementation do the rest */
	if (npages)
		kvm->arch.kvm_ops->flush_shadow_memslot(kvm, slot);
	spin_unlock(&kvm->mmu_lock);
}
...
BUILD_PTE_RANGE_OP(mkold, pte_mkold, kvm_pmd_mkold)

static int kvm_mkold_gpa_pt(struct kvm *kvm, gfn_t start_gfn,
				 gfn_t end_gfn)
{
	return kvm_mkold_pgd(kvm->arch.gpa_mm.pgd, start_gfn << PAGE_SHIFT,
				end_gfn << PAGE_SHIFT);
}
...
static int kvm_age_hva_handler(struct kvm *kvm, gfn_t gfn, gfn_t gfn_end,
			       struct kvm_memory_slot *memslot, void *data)
{
	return kvm_mkold_gpa_pt(kvm, gfn, gfn_end);
}
...
retry:
	...
	if (pmd_present(old_pmd)) {
		 ...
		 */
		if (!pmd_huge(old_pmd)) {
			++vcpu->stat.lvz_huge_merge_exits;
	...
	kvm_lvz_host_tlb_inv(vcpu, addr & PMD_MASK);
	set_pmd(pmd, *new_pmd);
	return 0;
}
			     ...
			     pte_t *out_entry, pte_t *out_buddy)
{
	...
	u32 exccode = (vcpu->arch.host_estat & CSR_ESTAT_EXC) >> CSR_ESTAT_EXC_SHIFT;
	...
	if ((exccode != EXCCODE_TLBRI) && (exccode != EXCCODE_TLBXI)) {
		err = kvm_map_page_fast(vcpu, gpa, write_fault, out_entry,
					      out_buddy);
		if (!err)
			goto out;
	}
				      ...
				      bool write_fault)
{
	...
	ret = kvm_map_page(vcpu, badvaddr, write_fault, NULL, NULL);
	if (ret)
		return ret;

	/* Invalidate this entry in the TLB */
	return kvm_lvz_host_tlb_inv(vcpu, badvaddr);
}
