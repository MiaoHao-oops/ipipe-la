 ...
 * for more details.
 *
 * KVM/LOONGISA: Stable timer emulation
 *
 * Copyright (C) 2020  Loongson Inc.  All rights reserved.
 ...
 * This can be used in conjunction with kvm_loongarch_freeze_timer() when timer
 * parameters need to be changed.
 *
 * It is guaranteed that a timer interrupt immediately after resume will be
 * handled. That case is already handled by kvm_loongarch_freeze_timer().
 */
static void kvm_resume_hrtimer(struct kvm_vcpu *vcpu, ktime_t now, u64 stable_timer)
{
	...
	*/
	...
	hrtimer_start(&vcpu->arch.swtimer, expire, HRTIMER_MODE_ABS);
}
...
enum hrtimer_restart kvm_count_timeout(struct kvm_vcpu *vcpu)
{
	...
	timer_cfg = kvm_read_sw_gcsr(vcpu->arch.csr, LOONGARCH_CSR_TCFG);
	if (timer_cfg & CSR_TCFG_PERIOD) {
		hrtimer_add_expires_ns(&vcpu->arch.swtimer, timer_cfg & CSR_TCFG_VAL);
...
void kvm_restore_timer(struct kvm_vcpu *vcpu)
{
	 ...
	 */
	timer_cfg = kvm_read_sw_gcsr(csr, LOONGARCH_CSR_TCFG);
	kvm_restore_hw_gcsr(csr, LOONGARCH_CSR_ESTAT);
	if (!(timer_cfg & CSR_TCFG_EN)) {
	...
	stable_timer = kvm_read_sw_gcsr(csr, LOONGARCH_CSR_TVAL);
	...
	if (expired) {
		if (timer_cfg & CSR_TCFG_PERIOD) {
			new_timertick = (delta - stable_timer) % (timer_cfg & CSR_TCFG_VAL);
		} else {
			new_timertick = 1;
		}
	} else {
	...
	new_timertick &= CSR_TCFG_VAL;
	write_gcsr_timercfg(timer_cfg);
	write_gcsr_timertick(new_timertick);
	if (expired)
 ...
 * @vcpu:       Virtual CPU.
 *
 * Save VZ guest timer state and switch to software emulation of guest
...
static ktime_t _kvm_save_timer(struct kvm_vcpu *vcpu, u64 *stable_timer)
{
	 ...
	 */
	end_stable_timer = read_gcsr_timertick();
...
void kvm_save_timer(struct kvm_vcpu *vcpu)
{
	...
	if (!(guestcfg & CSR_GCFG_TIT)) {
		...
		if (kvm_read_sw_gcsr(csr, LOONGARCH_CSR_TCFG) & CSR_TCFG_EN) {
			...
			kvm_write_sw_gcsr(csr, LOONGARCH_CSR_TVAL, stable_timer);
