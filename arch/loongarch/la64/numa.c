...
{
	unsigned int n, i;
	struct priv_mmap *nmp;
	struct loongsonlist_mem_map *lmp;

	if (loongson_sysconf.bpi_version >= BPI_VERSION_V3) {
		nmp = (struct priv_mmap *)g_mmap;
		for (i = 0; i < nmp->map_count; i++) {
			map[i].mem_type = nmp->map[i].mem_type;
			map[i].mem_start = nmp->map[i].mem_start;
			map[i].mem_size = nmp->map[i].mem_size;
		}
		n = nmp->map_count;
	} else {
		lmp = (struct loongsonlist_mem_map *)g_mmap;
		for (i = 0; i < lmp->map_count; i++) {
			map[i].mem_type = lmp->map[i].mem_type;
			map[i].mem_start = lmp->map[i].mem_start;
			map[i].mem_size = lmp->map[i].mem_size;
		}
		n = lmp->map_count;
	}
	return n;
