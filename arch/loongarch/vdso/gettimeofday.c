...
static __always_inline int do_realtime_coarse(struct timespec *ts,
					      const union loongarch_vdso_data *data)
{
	u32 start_seq;
...
static __always_inline u64 get_ns(const union loongarch_vdso_data *data)
{
...
{
	const union loongarch_vdso_data *data = get_vdso_data();
	struct timespec ts;
	int ret;
...
static __always_inline uid_t __do_getuid(const union loongarch_vdso_data *data)
{
	u32 start_seq;
	u32 cpu_id;
	uid_t uid;
