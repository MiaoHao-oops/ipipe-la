...
MODULE_DESCRIPTION("Loongson 3A4000 compatible  ACPI Processor P-States Driver");
MODULE_LICENSE("GPL");
...
#if defined(__mips__)
/*
 * Update clockevent frequency when the cpu frequency changes.
 */
static void ce_update_frequency(void *data)
{
	struct ce_update_data *ce_data = (struct ce_update_data *)data;

	if (ce_data)
		clockevents_update_freq(ce_data->cd, ce_data->new_freq);
}
#endif
	...
	if (val == CPUFREQ_POSTCHANGE) {
		...
		if (stable_timer_enabled) {
			...
			if (cur_boost_level != 0)
				new_freq = boost_freqs[cur_boost_level] * 1000 * 1000;
			else
				new_freq = core->normal_max_freq * 1000 * 1000;
		} else {
			cd = &per_cpu(mips_clockevent_device, cpu);
			new_freq = (freqs->new * 1000) >> 1;
		}

		ce_data.cd = cd;
		ce_data.new_freq = new_freq;
		if (cpu_online(freqs->cpu))
			smp_call_function_single(freqs->cpu, ce_update_frequency, &ce_data, 1);
#endif
		if (cur_boost_level != 0) {
			cpu_data[cpu].udelay_val = (unsigned int) (((int64_t)core->clock_scale[cur_boost_level] *
					...
					unsigned long val, void *data)
{
	...
	if (val == CPUFREQ_POSTCHANGE) {
		...
		clockevents_update_freq(cd, new_freq);
#endif
		if (cur_boost_level != 0) {
			cpu_data[cpu].udelay_val = (unsigned int) (((int64_t)core->clock_scale[cur_boost_level] *
...
static void do_set_freq_level(int cpu, int freq_level)
{
	...
	val = csr_readl(0x420);
#else
	iocsr_writel(message, 0x51c);
	val = iocsr_readl(0x420);
#endif
...
#else
		if (iocsr_readl(0x51c) & COMPLETE_STATUS) {
#endif
			ret = 0;
			break;
		}
...
static int loongson3_cpufreq_verify_policy(struct cpufreq_policy *policy)
{
...
static bool support_boost(void)
{
	...
	if(wait_for_ready_timeout(MAX_READY_TIMEOUT)) {
		...
		/* set 0x51c complete */
#if defined(__mips__)
		csr_writel(COMPLETE_STATUS, 0x51c);
#else
		iocsr_writel(COMPLETE_STATUS, 0x51c);
#endif
	} else {
...
static int set_boost(int state)
{
	struct cpufreq_policy *policy;
	int cpu;
