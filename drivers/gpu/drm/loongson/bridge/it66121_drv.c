...
static void it66121_hdcp_enable(struct bridge_phy *phy)
{
	bridge_phy_reg_mask_seq(phy, it66121_hdcp_seq,
				ARRAY_SIZE(it66121_hdcp_seq));
}

static void it66121_get_status(struct bridge_phy *phy)
{
	...
	DRM_DEV_DEBUG(to_dev(phy), "[SYS]=0x%x, [CLK]=0x%x\n", sys, clk);
}
...
const char *it66121_get_hpd_status_name(enum hpd_status status)
{
...
static enum hpd_status it66121_get_hpd_status(struct bridge_phy *phy)
{
	...
	if (test_bit(SYS_STATUS_HPD_POS, (unsigned long *)&val) ==
	    SYS_HPD_PLUG_ON) {
		DRM_DEV_DEBUG(to_dev(phy), "IT66121 hpd status: Connected\n");
		return hpd_status_plug_on;
	}

	DRM_DEV_DEBUG(to_dev(phy), "IT66121 hpd status: Disconnected\n");
	return hpd_status_plug_off;
}

static int it66121_int_enable_hpd(struct bridge_phy *phy)
{
	int ret;

	ret = regmap_update_bits(phy->phy_regmap,
	...
	it66121_get_status(phy);
	return 0;
}

static int it66121_int_enable_all(struct bridge_phy *phy)
{
...
static int it66121_int_enable(struct bridge_phy *phy, enum int_type interrupt)
{
	switch (interrupt) {
	case interrupt_hpd:
		it66121_int_enable_hpd(phy);
		break;
	case interrupt_all:
		it66121_int_enable_all(phy);
		break;
	default:
...
static int it66121_int_disable_hpd(struct bridge_phy *phy)
{
	int ret;
...
static int it66121_int_disable_all(struct bridge_phy *phy)
{
...
static int it66121_int_disable(struct bridge_phy *phy, enum int_type interrupt)
{
	switch (interrupt) {
	case interrupt_hpd:
		it66121_int_disable_hpd(phy);
		break;
	case interrupt_all:
		it66121_int_disable_all(phy);
		break;
	default:
		DRM_DEV_ERROR(to_dev(phy), "Invalid interrupt type");
		return -EINVAL;
...
static int __do_clear_int(struct bridge_phy *phy)
{
	int ret;
	unsigned int val;

	regmap_read(phy->phy_regmap, IT66121_REG_SYS_STATUS, &val);
	ret = regmap_update_bits(phy->phy_regmap, IT66121_REG_SYS_STATUS,
...
static int __it66121_int_clear(struct bridge_phy *phy, unsigned int num,
			       unsigned int mask)
{
	int ret;

	ret = regmap_update_bits(phy->phy_regmap, IT66121_REG_INT_CLEAR(num),
				 mask, 1U);
	if (ret) {
...
static void it66121_int_clear_all(struct bridge_phy *phy)
{
	struct regmap *phy_regmap;

	phy_regmap = phy->phy_regmap;
	DRM_DEV_DEBUG(to_dev(phy), "Clear all interrupts\n");

	regmap_write(phy_regmap, IT66121_REG_INT_CLEAR(INT_CLEAR0),
		     INT_CLEAR0_MSK);
	regmap_write(phy_regmap, IT66121_REG_INT_CLEAR(INT_CLEAR1),
		     INT_CLEAR1_MSK);
	__do_clear_int(phy);
}
...
static int it66121_int_clear(struct bridge_phy *phy, enum int_type interrupt)
{
	switch (interrupt) {
	case interrupt_hpd:
		__it66121_int_clear(phy, INT_CLEAR0, INT_CLEAR_HPD);
		break;
	case interrupt_all:
		it66121_int_clear_all(phy);
		break;
	default:
		DRM_DEV_ERROR(to_dev(phy), "Invalid interrupt type");
		return -EINVAL;
 ...
 */
static void __it66121_ddc_cmd(struct bridge_phy *phy, enum it66121_ddc_cmd cmd)
{
	regmap_update_bits(phy->phy_regmap, IT66121_REG_DDC_CMD,
			   IT66121_REG_DDC_CMD_MSK, cmd);
}

static int it66121_ddc_hdcp_reset(struct bridge_phy *phy)
{
	int ret;

	ret = regmap_update_bits(phy->phy_regmap, IT66121_REG_RESET,
...
{
	__it66121_ddc_cmd(phy, CMD_ABORT);
			     ...
			     struct drm_connector *connector)
{
	...
	if (edid) {
		...
		DRM_DEV_DEBUG(to_dev(phy), "Update %d edid method\n", count);
	} else {
 ...
 */
static int it66121_unregister_irq(struct bridge_phy *phy)
{
	struct bridge_resource *it66121_res;

	it66121_res = phy->res;
	if (gpio_is_valid(it66121_res->irq_gpio)) {
		free_irq(gpio_to_irq(it66121_res->irq_gpio), phy);
...
static int it66121_int_init(struct bridge_phy *phy)
{
	int ret;
	struct regmap *regmap;
	...
	ret = it66121_int_enable_hpd(phy);
	DRM_DEV_DEBUG(to_dev(phy), "Enable it66121 HPD interrupt and irq\n");
...
int bridge_phy_it66121_init(struct bridge_resource *res)
{
	...
	it66121_int_init(it66121_phy);
	res->ldev->mode_info[res->display_pipe_index].connector = NULL;
...
{
	it66121_hdcp_enable(phy);
	it66121_int_enable_all(phy);
	it66121_ddc_hdcp_reset(phy);
	it66121_afe_low(phy);
	it66121_int_disable_all(phy);
	it66121_unregister_irq(phy);
	it66121_int_disable_hpd(phy);
	kfree(phy);
	phy = NULL;
