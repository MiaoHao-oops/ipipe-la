...
{
	regmap_update_bits(phy->phy_regmap, LT8618_REG_ENABLE,
...
static enum hpd_status lt8618_get_hpd_status(struct bridge_phy *phy)
{
	...
	if (test_bit(LINK_STATUS_OUTPUT_DC_POS, (unsigned long *)&val) ==
	    LINK_STATUS_STABLE) {
		DRM_DEV_DEBUG(to_dev(phy), "LT8618 hpd status: Connected\n");
		return hpd_status_plug_on;
	}

	DRM_DEV_DEBUG(to_dev(phy), "LT8618 hpd status: Disconnected\n");
...
{
	regmap_update_bits(phy->phy_regmap, LT8618_REG_DDC_CMD,
