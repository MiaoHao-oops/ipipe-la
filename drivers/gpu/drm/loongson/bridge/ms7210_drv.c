...
static enum hpd_status ms7210_get_hpd_status(struct bridge_phy *phy)
{
	...
	if (test_bit(LINK_STATUS_OUTPUT_DC_POS, (unsigned long *)&val) ==
	    LINK_STATUS_STABLE) {
		DRM_DEV_DEBUG(to_dev(phy), "ms7210 hpd status: Connected\n");
		return hpd_status_plug_on;
	}

	DRM_DEV_DEBUG(to_dev(phy), "ms7210 hpd status: Disconnected\n");

	return hpd_status_plug_off;
}

static int ms7210_int_enable_hpd(struct bridge_phy *phy)
{
	regmap_update_bits(phy->phy_regmap, MS7210_REG_INT_CTRL,
			   INT_CTRL_HPD_MSK, INT_HPD_ENABLE);
	return 0;
}

static int ms7210_int_disable_hpd(struct bridge_phy *phy)
{
	regmap_update_bits(phy->phy_regmap, MS7210_REG_INT_CTRL,
			   INT_CTRL_HPD_MSK, INT_HPD_DISABLE);
	return 0;
}
static int ms7210_int_clear_hpd(struct bridge_phy *phy)
{
	regmap_update_bits(phy->phy_regmap, MS7210_REG_INT_CLEAR,
...
static int ms7210_int_init(struct bridge_phy *phy)
{
	int ret;
	...
	ret = ms7210_int_enable_hpd(phy);
	DRM_DEV_DEBUG(to_dev(phy), "Enable ms7210 HPD interrupt\n");
				      ...
				      const char *buf, size_t count)
{
	...
	if (sysfs_streq(buf, "0"))
		ms7210_int_enable_hpd(phy);
	if (sysfs_streq(buf, "1"))
		ms7210_int_disable_hpd(phy);
	if (sysfs_streq(buf, "2"))
		ms7210_int_clear_hpd(phy);
