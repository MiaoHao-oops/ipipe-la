...
#include "gsgpu/gsgpu_dc_device.h"
				     ...
				     int atomic)
{
	...
	ls_mm_wreg(ldev, LS_FB_STRI_REG + crtc_offset, ((pitch + 255) & ~255));
				   ...
				   struct drm_modeset_acquire_ctx *ctx)
{
	...
	val = ls_mm_rreg(ldev, DC_FB_CFG_REG + reg_offset);

	val |= BIT(12);

	ls_mm_wreg(ldev, DC_FB_CFG_REG + reg_offset, val);
