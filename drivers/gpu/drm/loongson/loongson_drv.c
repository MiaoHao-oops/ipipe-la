...
int loongson_modeset_init(struct loongson_device *ldev)
{
	...
	for (i = 0; (i < ldev->num_crtc && i < LS_MAX_MODE_INFO); i++) {
		...
		if (ls_crtc && ldev->chip == dc_7a2000)
			gsgpu_dc_connector_init(ldev, i);
		else if (ls_crtc && ldev->chip != dc_7a2000)
			loongson_bridge_bind(ldev, i);
		else
...
static int loongson_load_kms(struct drm_device *dev, unsigned long flags)
{
	...
	if (ldev->chip == dc_7a2000) {
		...
		gsgpu_dc_hdmi_init(ldev);
		gsgpu_dc_hotplug_config(ldev);
	}
	return 0;
...
static int loongson_drm_plat_unregister(struct platform_device *pdev)
{
	...
	struct loongson_device *ldev = dev->dev_private;
	u32 vaddr = phys_to_virt(ldev->mc.vram_start);
	free_pages(vaddr, 11);
	drm_put_dev(dev);
	return 0;
}
