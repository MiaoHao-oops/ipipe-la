...
irqreturn_t loongson_irq_handler(int irq, void *arg)
{
	...
	if (ldev->chip == dc_7a2000) {
		if (reg_val & HDMI_INT_HOTPLUG0_CTL) {
			queue_hp = true;
			ls_mm_wreg(ldev, LS_FB_INT_REG, reg_val | HDMI_INT_HOTPLUG0_CTL);
		}

		if (reg_val & HDMI_INT_HOTPLUG1_CTL) {
			queue_hp = true;
			ls_mm_wreg(ldev, LS_FB_INT_REG, reg_val | HDMI_INT_HOTPLUG1_CTL);
		}
		...
		if (reg_val & VGA_INT_HOTPLUG_CTL) {
			...
			ls_mm_wreg(ldev, LS_FB_INT_REG, reg_val | VGA_INT_HOTPLUG_CTL);
