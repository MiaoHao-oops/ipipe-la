				     ...
				     struct ttm_mem_type_manager *man)
{
	...
	case TTM_PL_VRAM:
		...
		man->gpu_offset = ldev->mc.vram_start;
...
static inline struct loongson_bo *loongson_bo(struct ttm_buffer_object *bo)
{
				       ...
				       struct ttm_mem_reg *mem)
{
	...
	case TTM_PL_VRAM:
		if (ldev->gpu_pdev)
			mem->bus.base = ldev->mc.aper_base;
		else
			mem->bus.base = ldev->mc.vram_start;
 ...
 * NULL: out of memeory
 */
static struct ttm_tt *loongson_ttm_tt_create(struct ttm_buffer_object *bo,
					     uint32_t page_flags)
{
	struct ttm_tt *tt;
