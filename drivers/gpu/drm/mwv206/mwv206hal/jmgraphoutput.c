...
{
	volatile unsigned int value;
	unsigned int palette_ram_sel_regaddr, palette_wr_data_regaddr, palette_fifo_aclr_regaddr,
		     palette_rgb_sel_regaddr, palette_fifo_used_regaddr;
	int rgb, curPaletteRam;
	unsigned int palette_enable_regaddr;
	GLJ_TICK    nTick;
	int nWrited = 0;
	...
	case 1:
		palette_ram_sel_regaddr = 0x947C + OUTPUT_REG_OFFSET(channel);
		palette_wr_data_regaddr = 0x9480 + OUTPUT_REG_OFFSET(channel);
		palette_fifo_aclr_regaddr = 0x9484 + OUTPUT_REG_OFFSET(channel);
		palette_rgb_sel_regaddr = 0x9488 + OUTPUT_REG_OFFSET(channel);
		palette_fifo_used_regaddr = 0x948C + OUTPUT_REG_OFFSET(channel);
		break;
