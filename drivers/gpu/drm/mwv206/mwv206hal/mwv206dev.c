...
int FUNC206HAL315(V206DEV025 *pDev, int memfreq)
{
	...
	FUNC206LXDEV154(pDev, 0,
	 ...
	 if (mwv206_vbios_cmp(pDev, 3, 3, 0, &accurate) >= 0 && accurate) {
		...
		int chiptype     = pDev->chiptype;
		int iscommercial = (chiptype == 0x9 || chiptype == 0xa
				|| chiptype == 0xf || chiptype == 0x10);

		FUNC206HAL134(pDev, V206IOCTL010, &current_memfreq);

		if (iscommercial) {
			if (current_memfreq == 266 && current_memfreq != memfreq) {
				return mwv206_ddr_config(pDev, memfreq);
			}
		} else {
...
void FUNC206HAL221(struct pci_dev *V206DEV103)
{
	...
	for (bar = 0; bar < 7; bar++) {
		...
		if (mwv206_is_sw64_421(pDev->V206DEV028)) {
			phys &= ~0xf000000000;
		}
#endif

#if defined(__loongarch__)
	#if ((LINUX_VERSION_CODE >= KERNEL_VERSION(4, 19, 0)) && (LINUX_VERSION_CODE <= KERNEL_VERSION(4, 19, 299)))

		if (syscall_get_arch() == 0xc0000102) {
