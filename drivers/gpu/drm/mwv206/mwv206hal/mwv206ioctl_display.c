...
static void mwv206_shadow_try_update(V206DEV025 *pDev, int crtc)
{
...
static void mwv206_shadow_sync(struct work_struct *work)
{
	...
	FUNC206HAL369(pDev, 1);
	if ((sc->update_pending || sc->display_pending) && !pDev->pm.V206DEV109 && !sc->drawing) {
		mwv206_shadow_try_update(pDev, sc->V206DEV143.V206FB011);

		sc->display_pending = sc->update_pending;
	}
	FUNC206HAL369(pDev, 0);
...
void mwv206_shadow_try_sync(V206DEV025 *pDev)
{
	...
	for (crtc = 0; crtc < 4; crtc++) {
		...
		mwv206_shadow_try_update(pDev, crtc);
	}
	FUNC206HAL369(pDev, 0);
}
...
int mwv206_resethdmiphy(V206DEV025 *pDev)
{
	int formerval, V206IOCTLCMD010, tempval;
...
int FUNC206HAL396(V206DEV025 *pDev, long arg, int legacy_mode)
{
