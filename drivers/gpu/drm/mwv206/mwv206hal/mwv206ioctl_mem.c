...
int V206IOCTL134(V206DEV025 *pDev, long arg)
{
	...
	case V206IOCTL023: {
		...
		if (V206CTYPE006(pDev->V206DEV028) || mwv206_is_sw64_421(pDev->V206DEV028)) {
			for (lines = 0; lines < memaccess->vsize; lines++) {

				ret = FUNC206HAL475(V206DEV031, pDev->V206DEV104, size, MWV206_MEMACCESS_CHECK_SLICE);
				if (ret != 0) {
					V206KDEBUG002("MWV206MEM_READ2D: copy_to_user error, ret = %d.", ret);
					goto FUNC206HAL020;
				}
				memcpy_fromio((void __user *)V206DEV031, (void *)V206IOCTLCMD009, size);
	...
	case V206IOCTL024: {
		...
		if (V206CTYPE006(pDev->V206DEV028) || mwv206_is_sw64_421(pDev->V206DEV028)) {
			for (lines = 0; lines < memaccess->vsize; lines++) {
				...
				ret = FUNC206HAL474(V206DEV031, pDev->V206DEV104, size, MWV206_MEMACCESS_CHECK_SLICE);
				if (ret != 0) {
					V206KDEBUG002("MWV206MEM_WRITE2D: copy_from_user error, ret = %d.", ret);
					goto FUNC206HAL020;
				}
				memcpy_toio((void *)V206IOCTLCMD009, (void __user *)V206DEV031, size);
				V206DEV031 += memaccess->memstride;
				V206IOCTLCMD009 += memaccess->mwv206stride;
			}
		} else
...

		{
			for (lines = 0; lines < memaccess->vsize; lines++) {
				...
				ret = FUNC206HAL476(V206DEV031, pDev->V206DEV104, V206IOCTLCMD009, size, MWV206_MEMACCESS_CHECK_SLICE);
				if (ret != 0) {
					V206KDEBUG002("MWV206MEM_WRITE2D: copy_from_user error, ret = %d.", ret);
					goto FUNC206HAL020;
				}
	...
	case V206IOCTL020: {
...
#if defined(CONFIG_KYLINOS_DESKTOP) || defined(CONFIG_KYLINOS_SERVER)
		if (V206CTYPE006(pDev->V206DEV028) || mwv206_is_sw64_421(pDev->V206DEV028)) {
			V206DEV005("[cpu-type] MWV206MEM_READ: ft1500a/sw64_421.\n");

			ret = FUNC206HAL475(V206DEV031, pDev->V206DEV104, size, MWV206_MEMACCESS_CHECK_SLICE);
			if (ret != 0) {
				V206KDEBUG002("MWV206MEM_READ: copy_to_user error.");
				goto FUNC206HAL020;
			}
			memcpy_fromio((void __user *)V206DEV031, (void *)V206IOCTLCMD009, size);
		} else
		...
		{
			V206DEV005("[cpu-type] MWV206MEM_READ: not ft1500a/sw64_421.\n");
			ret = copy_to_user((void __user *)V206DEV031, (void *)V206IOCTLCMD009, size);
			if (ret != 0) {
	...
	case V206IOCTL021: {
		...
		if (V206CTYPE006(pDev->V206DEV028) || mwv206_is_sw64_421(pDev->V206DEV028)) {
			...
			V206DEV005("[cpu-type] MWV206MEM_WRITE: ft1500a/sw64_421.\n");
			ret = FUNC206HAL474(V206DEV031, pDev->V206DEV104, size, MWV206_MEMACCESS_CHECK_SLICE);
			if (ret != 0) {
...
#else
		{
			V206DEV005("[cpu-type] MWV206MEM_WRITE: not ft1500a/sw64_421.\n");
			ret = copy_from_user((void *)V206IOCTLCMD009, (void __user *)V206DEV031, size);
			if (ret != 0) {
				V206KDEBUG002("MWV206MEM_WRITE: copy_from_user error, ret = %d.", ret);
				goto FUNC206HAL020;
...
#else
			{
				V206DEV005("[cpu-type] MWV206MEM_SET: not ft1500a/sw64_421.\n");
		...
		const unsigned char *V206IOCTLCMD006, unsigned int V206IOCTLCMD009, unsigned int size)
{
	...
	if (V206CTYPE006(pDev->V206DEV028) || mwv206_is_sw64_421(pDev->V206DEV028)) {
		V206DEV005("[cpu-type] mwv206MemRead: ft1500a/sw64_421.\n");
		memcpy_fromio((void *)V206IOCTLCMD006, (void *)V206KG2D026, size);
	} else
