	...
	for (i = 0; i < chancnt; i++) {
		for (j = 0; j < 2; j++) {
			...
			pDev->V206DEV090.V206DEV168[i][j] = FUNC206LXDEV009(&(pDev->V206DEV103->dev),
					&pDev->V206DEV090.V206DEV169[i][j], (1024*1024*4));
...
void FUNC206HAL324(V206DEV025 *pDev)
{
	...
	for (i = 0; i < chancnt; i++) {
		for (j = 0; j < 2; j++) {
			...
			if (vaddr) {
				FUNC206LXDEV100(&(pDev->V206DEV103->dev), (1024*1024*4),
...
int FUNC206HAL329(V206DEV025 *pDev,
		int op, int chan,
		...
		unsigned char *V206DEV031, int cpustride, int width, int height)
{
	...
	case V206IOCTL021:

		{
			...
			pvaddr = vaddr;
			for (i = 0; i < height; i++) {
				ret = copy_from_user(pvaddr, (void __user *)V206DEV031, width);
				if (ret != 0) {
...
int FUNC206HAL328(V206DEV025 *pDev, long arg)
{
	...
	w = width > (1024*1024*4) ? (1024*1024*4) : width;
	h = (1024*1024*4) / w;
