		...
		struct mwv206_port_config *port, V206IOCTL161 *p_mode)
{
	...
	FUNC206HAL396(priv, (long)&V206FB004, 0);
		...
		struct mwv206_port_config *port)
{
	...
	if (fb_parse_edid(port->edid,  &var)
			|| var.xres > V206FB005->maxwidth
			|| var.yres > V206FB005->maxheight) {
...
static void mwv206fb_pick_modes(V206DEV025 *pDev, struct V206DEV026 *V206FB005, struct fb_modelist *modelist)
{
	...
	list_for_each(pos, &modelist->list) {
		...
		if (d < distance) {
			...
			best = offset < ml->mode.refresh ? ml : best;
		}
	}
	if (best) {
...
static void mwv206fb_post_init(V206DEV025 *priv)
{
	...
	port_for_each(&priv->V206DEV105, port) {
		crtc = V206FB005->zoom_port == port ? MWV206_CRTC_ZOOM : MWV206_CRTC_PRIMARY;
		mode = V206FB005->zoom_port == port ? 1 : 0;
		if (port_is_hdmi(&priv->V206DEV105, port)) {
			mwv206fb_sethdmimode(priv, crtc, port, &V206FB005->V206DEV183[mode]);
		}
		FUNC206LXDEV084(priv, crtc, port);
	}

	mwv206_resethdmiphy(priv);
