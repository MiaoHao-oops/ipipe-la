...
static int ls2k_mci_probe(struct platform_device *pdev)
{
	...
	*(volatile unsigned int *)(host->base + SDIINTEN) = 0x1ff;
	...
	if (pdev->dev.of_node) {
		of_property = (__be32 *)of_get_property(pdev->dev.of_node, "clock-frequency", NULL);
		if (of_property != 0)
			host->clk_rate = of_read_number(of_property, 2);
	}
	...
	/* card_detect */
	ret = mmc_gpiod_request_cd(mmc, "cd", 0, true, 0, &cd_gpio_invert);
	if (ret) {
		dev_err(mmc_dev(mmc), "failed to request card-detect gpio!\n");
		ret = -EINVAL;
		goto probe_iounmap;
	}
...
#ifndef CONFIG_LOONGARCH
#define LS2K_GPIO_INTEN_BASE  TO_UNCAC(0x1fe10530)
	cd_gpio_num  = desc_to_gpio(ctx->cd_gpio);
	local_irq_save(flags);
	val = readq((void *)LS2K_GPIO_INTEN_BASE);
	val |= (0x1 << cd_gpio_num);
	writeq(val, (void *)LS2K_GPIO_INTEN_BASE);
	local_irq_restore(flags);
