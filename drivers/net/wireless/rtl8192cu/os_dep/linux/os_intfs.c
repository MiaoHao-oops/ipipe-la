...
#endif
int rtw_AcceptAddbaReq =
	_TRUE;// 0:Reject AP's Add BA req, 1:Accept AP's Add BA req.
...
MODULE_PARM_DESC(rtw_adaptor_info_caching_file_path,
...
uint loadparam(_adapter *padapter,  _nic_hdl	pnetdev)
{
	...
	registry_par->hwpdn_mode = (u8)
				   rtw_hwpdn_mode;//0:disable,1:enable,2:by EFUSE config
...
#ifdef CONFIG_ADAPTOR_INFO_CACHING_FILE
	snprintf(registry_par->adaptor_info_caching_file_path, PATH_LENGTH_MAX,
...
static struct net_device_stats *rtw_net_get_stats(struct net_device
		*pnetdev)
{
	_adapter *padapter = (_adapter *)rtw_netdev_priv(pnetdev);
...
struct net_device *rtw_init_netdev(_adapter *old_padapter)
{
	...
	if (old_padapter != NULL)
		pnetdev = rtw_alloc_etherdev_with_old_priv(sizeof(_adapter),
				(void *)old_padapter);
	else
...
u32 rtw_start_drv_threads(_adapter *padapter)
{
	...
	padapter->recvThread = kthread_run(rtw_recv_thread, padapter,
					   "RTW_RECV_THREAD");
...
#endif //CONFIG_CONCURRENT_MODE
	{
		...
		if (IS_ERR(padapter->cmdThread))
			...
			_rtw_down_sema(
				&padapter->cmdpriv.terminate_cmdthread_sema); //wait for cmd_thread to run
	}


#ifdef CONFIG_EVENT_THREAD_MODE
	padapter->evtThread = kthread_run(event_thread, padapter,
					  "RTW_EVENT_THREAD");
...
u8 rtw_reset_drv_sw(_adapter *padapter)
{
	...
	adapter_to_dvobj(padapter)->pusbdev->autosuspend_disabled =
...
int _netdev_vir_if_open(struct net_device *pnetdev)
{
	...
	if (primary_padapter->bup == _FALSE
	    || primary_padapter->hw_init_completed == _FALSE) {
	...
	if (padapter->bup == _FALSE && primary_padapter->bup == _TRUE &&
		...
		_rtw_memcpy(padapter->HalData, primary_padapter->HalData,
			    padapter->hal_data_sz);
		...
		rtw_start_mbssid_cam(
			padapter);//start mbssid_cam after bup = _TRUE & hw_init_completed = _TRUE
	...
	//if(check_fwstate(pmlmepriv, WIFI_STATION_STATE|WIFI_ADHOC_STATE) == _TRUE)
	{
...
#endif  // (LINUX_VERSION_CODE <= KERNEL_VERSION(2, 6, 35))
		{
			...
			if (br_netdev) {
				...
				printk("%s()-%d: dev_get_by_name(%s) failed!", __FUNCTION__, __LINE__,
...
netdev_open_normal_process:
...
#ifdef CONFIG_CONCURRENT_MODE
	{
		...
		if (sec_adapter && (sec_adapter->bup == _FALSE
				    || sec_adapter->hw_init_completed == _FALSE))
...
int pm_netdev_open(struct net_device *pnetdev, u8 bnormal)
{
	...
	if (bnormal)
		...
		status = (_SUCCESS == ips_netdrv_open((_adapter *)rtw_netdev_priv(
				pnetdev))) ? (0) : (-1);
#endif
