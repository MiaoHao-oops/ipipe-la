...
static acpi_status __init generic_acpi_handle_locate_callback(
		acpi_handle handle,
...
static int __init setup_acpi_notify(struct generic_struct *sub_driver)
{
	...
	rc = acpi_bus_get_device(*sub_driver->acpi->handle,
			&sub_driver->acpi->device);
	if (rc < 0) {
	...
	status = acpi_install_notify_handler(*sub_driver->acpi->handle,
			...
			dispatch_acpi_notify, sub_driver);
...
static int __init register_generic_subdriver(struct generic_struct *sub_driver)
{
	...
	sub_driver->acpi->driver = kzalloc(sizeof(struct acpi_driver),
				GFP_KERNEL);
	...
	sprintf(sub_driver->acpi->driver->name, "%s_%s",
			LSACPI_NAME, sub_driver->name);
...
static int event_init(struct generic_struct *sub_driver)
{
	int ret;
