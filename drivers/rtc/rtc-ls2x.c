...
static u32 ls2x_rtc_handler(void *id)
{
	...
	temp = readw(ACPI_FOR_RTC_WAKEUP_BASE);
	...
	writel(RTC_STS_STATUS_BIT, ACPI_FOR_RTC_STATUS_BASE);
...
static int ls2x_rtc_setaie(struct device *dev, unsigned int enabled)
{
	...
	/* set acpi rtc wakeup */
	temp = readl(ACPI_FOR_RTC_WAKEUP_BASE);
	if (enabled)
		temp |= RTC_STS_WAKEUP_BIT;
	else
		temp &= ~RTC_STS_WAKEUP_BIT;
	...
	do {
		writel(temp, ACPI_FOR_RTC_WAKEUP_BASE);
	} while (readl(ACPI_FOR_RTC_WAKEUP_BASE) != temp && !!(retry--));
...
static int ls2x_rtc_read_alarm(struct device *dev, struct rtc_wkalrm *alrm)
{
	...
	alrm_en = readl(ACPI_FOR_RTC_WAKEUP_BASE) & RTC_STS_WAKEUP_BIT;
